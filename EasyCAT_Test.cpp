...
uint32_t counter=0;
	while (1)
	{  
	 	    unsigned char Status = EASYCAT.MainTask();					// execute the EasyCAT task
		 EASYCAT.BufferIn.Byte[0]=counter;
		 EASYCAT.BufferIn.Byte[1]=counter;
		 EASYCAT.BufferIn.Byte[2]=counter;
		 EASYCAT.BufferIn.Byte[3]=counter;
		...
		//   cValue = EASYCAT.BufferOut.Byte[0];			// we read the input bit status reading the first byte from output buffer
		  ...
		  if (OutCount > 100 && Status == ESM_OP)
		  {
			...
			outputCounter++;
			counter++;
   printf("set = %d counter is %d\n\n ", EASYCAT.BufferOut.Byte[0],counter);	
		  }
		  
		  OutCount ++;

		  usleep(1);					// delay of 100mS
	}
}